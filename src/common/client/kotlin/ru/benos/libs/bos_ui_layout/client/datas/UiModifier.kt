...
import ru.benos.libs.bos_ui_layout.client.datas.events.UiKeyEvents
import ru.benos.libs.bos_ui_layout.client.datas.events.UiMouseEvents
import ru.benos.libs.bos_ui_layout.client.datas.events.UiNodeEvents
import ru.benos.libs.bos_ui_layout.client.enum.UiAlign

open class UiModifier(
    // Basic //

    val minSize: UiSizes  = UiSizes.ZERO,
    val maxSize: UiSizes? = null,
    val padding: UiInsets = UiInsets.ZERO,

    val overridePosition: Vector2i? = null,

    val stretchSize: UiStretchSize = UiStretchSize.WRAP,
    ...
    fun padding(left: Int = this.padding.left, top: Int = this.padding.top, right: Int = this.padding.right, bottom: Int = this.padding.bottom): UiModifier =
    ...
    protected fun stretchSize(width: IUiStretch = this.stretchSize.width, height: IUiStretch = this.stretchSize.height): UiModifier =
    ...
    fun resolveWidth(contentWidth: Int, availableWidth: Int): Int {
        ...
        return stretchSize.height.resolve(minSize.width.result, padding.horizontal, currentAvailable, contentWidth, availableWidth)
    }
